-- 0051_futures_schedule_windows.sql
--
-- Run this whole file in the Supabase SQL Editor after 0050.
--
-- Adds multi-window Schedule Windows support to Futures Strategy:
--   - Global filters: Trading Days, Expiry, Cycle Seconds, Armed.
--   - Window-specific filters: Start/End time, Entry Premium, Premium Range,
--     Pairs Count, Qty, Target Delta Band, Leverage, ATM Shift %, etc.
-- ============================================================================
...
alter table public.delta_strategy_settings
  add column if not exists schedule_windows jsonb not null default '[]'::jsonb;

alter table public.delta_strategy_settings
  add column if not exists entered_window_ids text[] not null default '{}';

comment on column public.delta_strategy_settings.schedule_windows is
  'List of schedule windows with window-specific trading parameters for Futures Strategy.';

-- ---------------------------------------------------------------------------
-- 2. delta_session_window: resolves active schedule window for current IST time
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 3. apply_delta_strategy with Schedule Windows support
-- ---------------------------------------------------------------------------
...
begin
  ...
  loop
    ...
    if v_mode = 'futures' then
      v_gp        := null;
      -- v_band_low & v_band_high already extracted from active window
