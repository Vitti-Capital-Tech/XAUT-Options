-- 0027_delta_dp_in_qty_units.sql
--
-- Run this whole file in the Supabase SQL Editor after 0026_delta_stop_loss_mark.sql.
--
-- Net portfolio delta was summed as Σ(net_qty × option-delta) with net_qty in raw
-- venue lots, so a book of 1000-lot legs read as ~1000× the delta the positions
-- table shows and the band (set in the book's own delta units) was breached on
-- every cycle. Scale the sum by the contract value so Δp reads in qty units — the
-- same unit §5.2's worked example and the band are written in — and divide the two
-- correction-sizing denominators by the contract value so a correction is still
-- sized in lots. The lot count each correction places is unchanged for any given
-- breach; only the breach test now reads in qty units, which is the fix.
--
-- Mirrors src/lib/deltaStrategy.ts (planCycle), which applies the same scaling.
