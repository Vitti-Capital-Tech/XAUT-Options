-- 0061_expiry_belongs_to_the_window.sql
--
-- Run this whole file in the Supabase SQL Editor after 0060.
--
-- The expiry moves from the account to the schedule window, as days to expiry.
--
--     0  the expiry settling today
--     1  the nearest expiry settling tomorrow or later
--     4  the nearest Friday weekly
--
-- Four is a label with a number on it, not four days of arithmetic: on a
-- Wednesday it still resolves to Friday, not Sunday. That is why the three
-- choices are fixed rather than a free integer — `2` has no agreed meaning and
-- would land wherever the calendar put it.
--
-- Why per window. A window that opens in the evening and one that opens at the
-- session open are not usually selling the same contract, and one account-wide
-- setting cannot say that. Each window now names its own, and the engine reads
-- `daysToExpiry` off the active window ahead of anything on the settings row.
--
-- The account columns stay as the fallback — `expiry_label` and `expiry_rule`
-- still decide for a `delta` book and for a futures book whose schedule_windows
-- is empty, which is the path the engine takes when there is no window to read.
-- The tab mirrors a window's choice into `expiry_rule` for exactly that reason,
-- so the two agree while a book has only the one synthesized window.
--
-- The global Expiry control is gone from the futures bar. It is not moved twice:
-- the options (`delta`) tab keeps its own, because that book has no windows.
--
...
begin
  ...
  -- The window's choice has to be read, or this migration did nothing.
  if v_src !~ 'daysToExpiry' then
    raise exception 'apply_delta_strategy does not read the window''s daysToExpiry';
  end if;
  ...
  raise log '0061: the active window''s DTE decides the expiry';
end;
$$;
