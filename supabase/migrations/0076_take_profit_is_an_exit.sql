-- 0076_take_profit_is_an_exit.sql
--
-- Run this whole file in the Supabase SQL Editor after 0075.
--
-- **A take-profit is an exit.** The leg did its job, the profit is banked, and
-- the window does not sell another one in its place.
--
-- 0074 made the opposite call, and it was wrong. It read a pair closed at
-- take-profit as a hole in the window's allocation and had the top-up refill it,
-- so a window ran at its configured size all session, re-selling everything it
-- banked. That is a different strategy from the one intended: take-profit is
-- where a position is *finished*, not where it is rolled.
--
-- ---------------------------------------------------------------------------
-- What the counter means now
-- ---------------------------------------------------------------------------
--
-- `pairs_open` goes back to counting pairs this window has **opened**, raised to
-- meet the book and never lowered to it — 0071's rule, and for a simpler reason
-- than 0071 had. There is no longer a distinction to draw between kinds of
-- close, because **nothing that closes is refilled**: take-profit, stop-loss, an
-- ATM exit past its budgets, a margin cut, an out-of-margin close. All the same
-- answer.
--
-- So the counter answers one question: how much of the window's allocation has
-- ever been opened. The top-up fills what an entry left behind — two pairs of
-- three on a thin chain, with the third arriving when the chain offers it — and
-- once all three have been opened that window is done buying, whatever happens
-- to the book afterwards.
--
-- It is still *raised* to meet the book rather than merely stamped, because a
-- book the counter never saw still has to be counted: one carried across a
-- migration, or one the adopt branch takes over.
--
-- ---------------------------------------------------------------------------
-- What this removes
-- ---------------------------------------------------------------------------
--
-- `pairs_retired` and its three increments — the ATM exit past both budgets, the
-- full margin cut, the out-of-margin close. All three existed only to tell the
-- top-up "do not refill this one", which is now the answer for every close, so
-- there is nothing left for them to distinguish.
--
-- The column is **kept and zeroed**, not dropped. Dropping it would break any
-- browser still running yesterday's bundle, which selects it by name, for no
-- gain — it is four bytes that nothing reads. Its comment says so.
--
-- ---------------------------------------------------------------------------
-- What is unchanged
-- ---------------------------------------------------------------------------
--
-- The top-up itself ([`0070`](0070_fill_the_pairs_the_window_asked_for.sql)) —
-- "two pairs filled, keep trying for the third" is still exactly what happens,
-- and is the whole of what the top-up does now.
--
-- The panel keeps showing **pairs on the book**, not the counter. That was the
-- honest half of 0074 and it stays: "Pairs 3 / 3" over a book holding one pair
-- was a readout describing its own bookkeeping, and the fix for that was the
-- display, not the behaviour.
--
-- 0075's ladder, buffer and pair gap are untouched.
-- ---------------------------------------------------------------------------

comment on column public.delta_strategy_settings.pairs_retired is
  'Vestigial since 0076. Nothing reads or increments it: no close is refilled any more, so there is no kind of close to distinguish. Kept, and reset to 0 with the rest, only so an older client that selects it by name still works.';

comment on column public.delta_strategy_settings.pairs_open is
  'Pairs this window has opened, raised to meet the book and never lowered to it. The top-up fills the shortfall an entry left behind; once the allocation has been opened the window is done buying, whatever closes afterwards.';
...
declare
  ...
  v_has_range     boolean;
  -- 0076: the size of the book the top-up is aiming at. Plain `pairs_count`
  -- again -- 0074's `v_retired` is gone with the rule that needed it.
  v_target_pairs  int;
...
begin
  ...
  loop
    ...
    if v_mode = 'futures' then
      ...
      -- 0076: `pairs_open` counts pairs this window has *opened*. It is raised to
      -- meet the book and never lowered to it.
      --
      -- 0074 measured it both ways and subtracted `pairs_retired` from the
      -- target, so that a pair closed at take-profit was refilled while one the
      -- ATM rule gave up on was not. That distinction is gone: **a take-profit is
      -- an exit.** The leg did its job, the profit is banked, and the window does
      -- not sell another one in its place. Nothing that closes is refilled --
      -- take-profit, stop-loss, ATM exit, margin cut, all the same.
      --
      -- So the counter only has to answer one question now: how much of the
      -- window's allocation has ever been opened. The top-up fills the shortfall
      -- an entry left behind -- two pairs of three on a thin chain, with the
      -- third arriving later -- and once all three have been opened, that window
      -- is done buying regardless of what happens to the book afterwards.
      --
      -- Raised rather than merely stamped, because a book the counter never saw
      -- still has to be counted: one carried across a migration, or one taken
      -- over by the adopt branch below.
      if coalesce(s.pairs_open, 0) < v_opened
        ...
        if v_desc is not null then
          ...
          if v_opened > 0 then
            ...
            raise log 'apply_delta_strategy: account % topped window % up to % of % pair(s) — %',
              ...
              coalesce(s.pairs_open, 0) + v_opened, v_target_pairs, v_desc;
            v_n := v_n + 1;
            continue;
          end if;
        end if;
    ...
    if v_mode = 'futures' then
      ...
      loop
        ...
        if not v_shifted then
          -- 0076: nothing to record. `pairs_open` counts what was opened and is
          -- never lowered, so a leg closed here is not a pair the top-up can see
          -- as owed -- which is the same answer take-profit now gets.
...
-- ---------------------------------------------------------------------------
-- Resolve every name at apply time, and prove the rule changed back
-- ---------------------------------------------------------------------------
do $$
declare
  v_eng   text;
...
begin
  ...
  -- 1. The counter is raise-only again, so no close can read as a pair still owed.
  if v_eng not like '%pairs_open = greatest(coalesce(pairs_open, 0), v_opened)%' then
    raise exception 'pairs_open is not raise-only — a closed pair can be refilled (0076)';
  end if;
  if v_eng like '%set pairs_open = v_opened,%' then
    raise exception 'the reconciliation still lowers pairs_open to the book (0074)';
  end if;

  -- 2. The top-up aims at the whole allocation, with nothing subtracted from it.
  if v_eng like '%v_pairs - coalesce(s.pairs_retired, 0)%' then
    raise exception 'the top-up still subtracts retired pairs from the target (0074)';
  end if;
  if v_eng not like '%coalesce(s.pairs_open, 0) < v_pairs%' then
    raise exception 'the top-up no longer compares the counter against pairs_count';
  end if;

  -- 3. And nothing retires a pair, because nothing is refilled.
  if v_eng like '%pairs_retired = coalesce(pairs_retired, 0) + 1%' then
    raise exception 'something still increments pairs_retired — it means nothing now';
  end if;

  -- Carried forward from 0075, 0073, 0072, 0070 and 0069.
  if v_pick not like '%when b.floor_val > 0 then b.floor_val%' then
    raise exception 'the picker no longer aims at the floor (0075)';
  end if;
  if v_eng not like '%v_prem_max := v_prem_max * (1 + v_buffer_pct / 100.0);%' then
    raise exception 'the premium buffer is gone (0075)';
  end if;
  if v_entry not like '%if v_max_gap > 0 and v_gap > v_max_gap then%' then
    raise exception 'the pair-gap cap is gone (0075)';
  end if;
  ...
  raise log '0076: take-profit is an exit — nothing that closes is sold back on';
end;
$$;

-- Clear what 0074 left behind, so no window starts this one short.
update public.delta_strategy_settings set pairs_retired = 0 where pairs_retired <> 0;
