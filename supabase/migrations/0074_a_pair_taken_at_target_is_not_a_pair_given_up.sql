-- 0074_a_pair_taken_at_target_is_not_a_pair_given_up.sql
--
-- Run this whole file in the Supabase SQL Editor after 0073.
--
-- The panel reads "Pairs 3 / 3" over a book holding one call and one put, and
-- the top-up will not refill it.
--
-- Both legs on that book were survivors of *different* pairs — one stamped
-- "Opening pair", the other "topping up the window's allocation" — which is the
-- signature of legs leaving one at a time rather than in pairs. That is
-- `apply_tpsl_triggers`: a separate engine, on its own schedule, closing each
-- short as its mark decays to `take_profit_mark`. It does not know the strategy
-- exists and never touches its bookkeeping.
--
-- ---------------------------------------------------------------------------
-- What 0071 got wrong, and why it looked right
-- ---------------------------------------------------------------------------
--
-- 0071 reconciled `pairs_open` against the book, raised to meet it and never
-- lowered to it, and stated the rule as a principle:
--
--     Raised, the counter catches up with a book it did not see. Lowered, it
--     would erase a decision the book no longer shows: an ATM exit that closed a
--     leg while the re-entry budget declined to replace it reads as "a pair still
--     owed", and the top-up would quietly refill it.
--
-- The reason is still right. The mechanism was not. "Never lower" is a proxy for
-- a question the counter cannot answer — *why* did the book shrink — and it
-- answers "the strategy gave up on that pair" and "take-profit banked it"
-- identically. Take-profit is by far the commoner of the two, and it is not a
-- decision to stop holding a pair at all. It is the pair working.
--
-- So a window that opened three pairs and banked two of them sat at 3 of 3 with
-- one pair on the table, declining to trade for the rest of its life, and the
-- panel agreed with the counter rather than with the position table.
--
-- ---------------------------------------------------------------------------
-- The fix: answer the question instead of working around it
-- ---------------------------------------------------------------------------
--
-- `pairs_retired` counts only pairs this engine deliberately closed and chose
-- not to replace. `pairs_open` then goes back to being what it says — pairs on
-- the book, measured every cycle, in both directions — and the target moves
-- instead:
--
--     pairs wanted on the book = pairs_count − pairs_retired
--
-- A take-profit lowers `pairs_open` and nothing else, so the top-up sees a
-- shortfall and fills it. An ATM exit past both its budgets lowers `pairs_open`
-- *and* raises `pairs_retired`, so the target falls with it and nothing is owed.
-- Same book, two histories, two answers — which is exactly what 0071 could not
-- express, and why it had to refuse to lower the counter at all.
--
-- ---------------------------------------------------------------------------
-- What retires a pair
-- ---------------------------------------------------------------------------
--
-- Three closes, and only these:
--
--   * An **ATM exit** with the shift budget spent and nothing sold back inside
--     the premium range. Both budgets declined; the side stays shut.
--   * A **margin cut** that takes a leg out entirely. The guard closed it
--     because the book was past its cap, and selling it straight back is the
--     same trade in a loop paying a spread each way.
--   * An **out-of-margin close** of the leg driving Δp. It was closed at a loss
--     precisely because it could not be carried, and margin recovers as the rest
--     of the book decays — without this it would come back the moment it did.
--
-- The two margin cases count only on a *full* close. A partial cut leaves the
-- strike on the book, so the pair count has not moved and there is nothing for
-- the top-up to see.
--
-- Not retired: take-profit and stop-loss, which belong to another engine and
-- another decision; a shift or a re-entry, which replaced the leg rather than
-- gave it up; and a flatten, which resets both counters along with everything
-- else.
--
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
--
-- A futures window now holds its configured size for as long as it is open:
-- take-profits are banked and the pair is sold again inside the premium range.
-- That is more premium collected and more turnover — and more fees — than the
-- behaviour it replaces, where the book wound down across the session. It is the
-- behaviour the panel has been claiming all along.
--
-- Orphan legs are left alone deliberately. A call whose partner took profit is
-- not closed and not re-paired; Δp is kept inside the band by the perpetual
-- whether the legs sit in pairs or not, and the top-up sells whole pairs rather
-- than chasing a match for a single leg.
-- ---------------------------------------------------------------------------
...
alter table public.delta_strategy_settings
  add column if not exists pairs_retired integer not null default 0;

comment on column public.delta_strategy_settings.pairs_retired is
  'Pairs the engine closed and chose not to replace (ATM exit past its budgets, full margin cut, out-of-margin close). Subtracted from pairs_count to give the size the top-up aims at. Take-profit does not retire a pair.';

comment on column public.delta_strategy_settings.pairs_open is
  'Pairs currently on the book for the traded expiry, measured every cycle. Since 0074 it moves in both directions; what the strategy gave up on is in pairs_retired.';
...
begin
  ...
  -- 1. The counter moves both ways now. 0071's guard asserted the opposite and
  --    is deliberately reversed here — see the header.
  if v_eng like '%pairs_open = greatest(coalesce(pairs_open, 0), v_opened)%' then
    raise exception 'pairs_open is still raise-only — take-profit cannot lower it (0074)';
  end if;
  if v_eng not like '%set pairs_open = v_opened,%' then
    raise exception 'the reconciliation no longer measures pairs_open off the book';
  end if;

  -- 2. And the target it is measured against moves with the retirements.
  if v_eng not like '%v_pairs - coalesce(s.pairs_retired, 0)%' then
    raise exception 'the top-up does not subtract retired pairs from the target';
  end if;

  -- 3. All three retiring closes, and no more than three. Take-profit lives in
  --    apply_tpsl_triggers and must never appear here.
  if (length(v_eng) - length(replace(v_eng, 'set pairs_retired = coalesce(pairs_retired, 0) + 1', '')))
     / length('set pairs_retired = coalesce(pairs_retired, 0) + 1') <> 3 then
    raise exception 'expected exactly three closes to retire a pair';
  end if;

  -- 4. Every reset clears both counters, or a window inherits the last one's
  --    retirements and opens short for the rest of its life.
  if (length(v_eng) - length(replace(v_eng, 'pairs_retired = 0', '')))
     / length('pairs_retired = 0') < 5 then
    raise exception 'not every flatten clears pairs_retired';
  end if;

  -- Carried forward from 0073, 0072, 0070 and 0069.
  ...
  raise log '0074: a pair taken at target is refilled; a pair given up on is not';
end;
$$;

-- What the next cycle will find, for the record. Read-only.
do $$
declare
  v record;
begin
  for v in
    select dss.account_id, dss.pairs_open, dss.pairs_retired, dss.pairs_count,
           coalesce(least(
             count(distinct p.symbol) filter (where p.contract_type = 'call_options'),
             count(distinct p.symbol) filter (where p.contract_type = 'put_options')
           ), 0) as on_book
    from public.delta_strategy_settings dss
    join public.accounts a on a.id = dss.account_id and a.kind = 'futures'
    left join public.positions p
           on p.account_id = dss.account_id and p.net_qty < 0
          and p.contract_type in ('call_options', 'put_options')
    group by dss.account_id, dss.pairs_open, dss.pairs_retired, dss.pairs_count
  loop
    raise log '0074: account % — on the book %, stamped %, retired %, of % configured — top-up will aim at %',
      v.account_id, v.on_book, v.pairs_open, v.pairs_retired, v.pairs_count,
      v.pairs_count - v.pairs_retired;
  end loop;
end;
$$;
