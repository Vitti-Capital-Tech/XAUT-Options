...
--
-- Run this whole file in the Supabase SQL Editor after 0027_delta_dp_in_qty_units.sql.
--
-- Two delta-strategy settings go away, because neither earns its place:
--
--   min_premium      Entries take the strike quoted closest to entry_premium, so a
--                    separate floor never decides anything the closest-to rule has
--                    not already decided.
--   band_delta_low   Band correction picked its strike by delta while every other
--   band_delta_high  sale picked by price. It now uses entry_premium too, so there
--                    is one price rule rather than two that have to agree.
--
-- The floor arguments to delta_sell_entry and delta_pick_premium stay in those
-- signatures and are passed 0 — no premium is ever below zero, so the check is
-- inert. Leaving the signatures alone keeps this migration to one function body.
--
-- Order matters: apply_delta_strategy is recreated first so it no longer reads the
-- columns, and only then are the columns and delta_pick_delta dropped.
--
-- Mirrors src/lib/deltaStrategy.ts, which drops the same two settings.
...
-- The by-delta picker has no caller left.
drop function if exists public.delta_pick_delta(text, text, numeric, numeric, numeric);

alter table public.delta_strategy_settings
  drop column if exists min_premium,
  drop column if exists band_delta_low,
  drop column if exists band_delta_high;
