-- 0067_the_expiry_day_is_the_ist_day.sql
--
-- Run this whole file in the Supabase SQL Editor after 0066.
--
-- The delta engine resolved `today`, `tomorrow` and `friday` against the UTC
-- date while every other clock in the strategy runs on IST:
--
...
-- yesterday's, and the three rules answer for a day the account has already
-- left.
--
-- `today` and `friday` survived that by accident. An XAUT option settles at
-- 16:00 UTC on its label's date -- confirmed against /v2/products, and it is
-- what the `+ interval '16 hours'` liveness filter beside each of these encodes
-- -- so by 18:30 UTC, which is 00:00 IST, yesterday's contract is already two
-- and a half hours dead and the filter drops it. The rule then matches nothing
-- and falls through to "the nearest live expiry", which in that window is the
-- right one. Right answer, wrong reason, and only because the fallback existed.
--
-- `tomorrow` has no such luck, because it asks for `>= date + 1` rather than for
-- an exact date, and that is satisfiable by a contract the filter is happy with.
-- At 02:00 IST on the 6th the UTC date is the 5th, `>= the 6th` matches the 6th,
-- and the 6th is what IST calls *today*. A window asking for one day to expiry
-- was sold zero. No error, no log line, no readout disagreement -- the browser
-- computed the same wrong answer from the same UTC date, so the tab confirmed
-- it.
--
-- This is live rather than theoretical: `defaultScheduleWindow` starts a window
-- at 01:30, and a futures book here runs one inside that band.
--
-- The fix is to read the expiry day off the same clock as everything else. Note
-- that the label's date already *is* its IST date: settlement at 16:00 UTC is
-- 21:30 IST the same evening, so nothing about the labels has to be reinterpreted
-- -- only which day we compare them against.
--
-- `apply_strategy`, the auto engine, has always done it this way
-- (`to_char(now() at time zone 'Asia/Kolkata', 'DDMMYY')`), so this also ends a
-- disagreement between the two engines about what day it is.
--
-- 0062's engine verbatim with those three dates changed. The `+ interval '16
-- hours'` filters are untouched: an expiry's death is an instant, not a date,
-- and that comparison was always timezone-correct.
--
-- The browser's `resolveTargetExpiry` is changed in step, in the same commit.
-- ---------------------------------------------------------------------------
    ...
    if v_rule = 'fixed' then
      ...
      if v_rule = 'today' then
        ...
        where expiry_label ~ '^\d{6}$'
          and (to_date(expiry_label, 'DDMMYY')::timestamp at time zone 'UTC') + interval '16 hours' > now()
          and to_date(expiry_label, 'DDMMYY') = (now() at time zone 'Asia/Kolkata')::date
...
-- ---------------------------------------------------------------------------
-- Resolve every name at apply time, and prove the clock actually moved.
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- No expiry rule may read the UTC date any more. Deliberately the exact
  -- `(now() at time zone 'UTC')::date` form: the liveness filters beside these
  -- rules legitimately say `::timestamp at time zone 'UTC'` and must survive.
  if position('(now() at time zone ''UTC'')::date' in v_eng) > 0 then
    raise exception 'apply_delta_strategy still resolves an expiry against the UTC date';
  end if;
  if position('(now() at time zone ''Asia/Kolkata'')::date' in v_eng) = 0 then
    raise exception 'apply_delta_strategy does not resolve the expiry day on the IST clock';
  end if;

  -- The liveness filter is the half that must NOT have moved.
  ...
  -- Every window key the editor writes must still be read (0062's check).
  ...
  raise log '0067: the delta engine resolves today/tomorrow/friday on the IST clock';
end;
$$;
