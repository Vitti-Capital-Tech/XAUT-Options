...
alter table public.delta_strategy_settings
  drop constraint if exists delta_strategy_expiry_rule_chk;

alter table public.delta_strategy_settings
  add constraint delta_strategy_expiry_rule_chk
  check (expiry_rule in ('today', 'tomorrow', 'friday', 'nearest', 'fixed'));

comment on column public.delta_strategy_settings.expiry_rule is
      ...
      loop
        ...
        if v_used < coalesce(s.max_shifts, 1) then
          select * into v_pick from public.delta_pick_premium(
            ...
            v_leg.mark * (coalesce(s.shift_pct, 50) / 100.0),
            0, s.tie_break, v_leg.strike, r.account_id, s.max_notional_per_strike, v_spot, 0
          );
