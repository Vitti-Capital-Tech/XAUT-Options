...
--
-- Run this whole file in the Supabase SQL Editor after 0070.
--
-- After 0070 the panel reads "Pairs 0 / 3" on a book that is holding a pair, and
-- the top-up never runs.
--
-- 0070 added `pairs_open` and set it from the entry. That is right for every
-- book opened after 0070 applied, and wrong for every book that was already on
-- the table when it did: those rows took the column default of 0 while holding a
-- live pair. Nothing recounts them. The adopt branch is the only place that ever
-- derives the counter from the book, and it is gated on
-- `entered_day is distinct from v_day` — false for a book this same window
-- entered earlier in the session.
--
-- `open_window_id` (0069) has the identical gap one migration earlier: a book
-- opened before 0069 has no owning window, and the top-up is gated on the window
-- owning the book, so it was unreachable regardless of the counter.
--
-- Both halves read as one symptom, but they fail in opposite directions:
--
--   open_window_id null   the top-up cannot run at all. Nothing happens, which
--                         is what was reported.
--   pairs_open low        the top-up runs and asks for too much. Three pairs
--                         requested against a book already holding one is a
--                         four-pair strangle out of a window configured for
--                         three. Latent, because the first fault was masking it.
--
-- So this adds one reconciliation step, ahead of the adopt branch and therefore
-- ahead of every entry path: count the pairs actually short on the traded
-- expiry, raise `pairs_open` to meet that number, and let the window claim a
-- book its own stamps say it opened.
--
-- ---------------------------------------------------------------------------
-- Why `greatest`, and not "set it to what is on the book"
-- ---------------------------------------------------------------------------
--
-- The counter may be raised to meet the book and never lowered to it, because
-- the two directions mean completely different things:
--
--   book above counter   a migration, an adoption, a leg opened by hand. The
--                        bookkeeping is behind; catch it up, and the top-up then
--                        asks only for what is genuinely missing.
--   book below counter   an ATM exit closed a leg and the re-entry budget chose
--                        not to replace it. Lowering the counter here would have
--                        the top-up refill it on the next cycle, which makes
--                        `maxReentries` mean nothing at all.
--
-- That asymmetry is the entire rule. It is also why `pairs_open` cannot simply
-- be derived from the book on every cycle and the column dropped — the counter
-- has to remember a decision the book no longer shows.
--
-- ---------------------------------------------------------------------------
-- Ownership
-- ---------------------------------------------------------------------------
--
-- A window claims the book only on its own word: its id in
-- `entered_window_ids`, and `entered_day` equal to this session day. That is the
-- engine saying it opened a book here — a different thing from finding a book
-- and assuming it. A book nobody claims stays unowned and falls through to the
-- adopt branch, which is what 0069 intended for anything opened by hand.
--
-- No schema change. The engine is 0070's with the one block added, and the
-- browser's readout is changed in step so the panel stops disagreeing with the
-- engine about how many pairs are open.
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- 1. The reconciliation exists, and raises only.
  if v_eng not like '%pairs_open = greatest(coalesce(pairs_open, 0), v_opened)%' then
    raise exception 'the reconciliation does not raise pairs_open toward the book';
  end if;
  if v_eng ~ 'set[[:space:]]+pairs_open[[:space:]]*=[[:space:]]*v_opened\M' then
    raise exception 'the reconciliation assigns pairs_open outright — it must never lower it';
  end if;

  -- 2. Ownership is claimed on the window's own stamps, not on finding a book.
  if v_eng not like '%v_win_id = any(coalesce(s.entered_window_ids%' then
    raise exception 'the reconciliation claims a book without the window having entered';
  end if;
  if v_eng not like '%open_window_id is null and v_owns%' then
    raise exception 'the reconciliation overwrites an owning window';
  end if;

  -- 3. It runs ahead of adoption, and therefore ahead of every entry path.
  if position('Reconcile the bookkeeping against the book' in v_eng) >
     position('Adopt hand-opened books' in v_eng) then
    raise exception 'the reconciliation runs after adoption';
  end if;

  -- Carried forward from 0070.
  ...
  if v_eng not like '%v_pairs - coalesce(s.pairs_open, 0)%' then
    raise exception 'the top-up does not ask for the shortfall (0070)';
  end if;
  if v_eng not like '%v_brake%' then
    raise exception 'the top-up has no margin brake (0070)';
  end if;
  ...
  if v_entry not like '%join pu on pu.rn = ca.rn%' then
    raise exception 'delta_sell_entry pairs on the raw rank, not the filtered one (0070)';
  end if;
  ...
  raise log '0071: pairs_open is reconciled up to the book, never down to it';
end;
$$;

-- What the engine will find on the next cycle, for the record. Read-only.
do $$
declare
  v record;
begin
  for v in
    select dss.account_id, dss.pairs_open, dss.pairs_count, dss.open_window_id,
           coalesce(least(
    ...
    left join public.positions p
          ...
          and p.contract_type in ('call_options', 'put_options')
    group by dss.account_id, dss.pairs_open, dss.pairs_count, dss.open_window_id
  loop
    raise log '0071: account % — pairs_open %, on the book %, of % configured, window %',
      v.account_id, v.pairs_open, v.on_book, v.pairs_count,
      coalesce(v.open_window_id, '(unowned)');
  end loop;
end;
$$;
