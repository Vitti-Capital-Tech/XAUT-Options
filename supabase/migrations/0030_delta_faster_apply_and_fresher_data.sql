-- 0030_delta_faster_apply_and_fresher_data.sql
--
-- Run this whole file in the Supabase SQL Editor after 0029_delta_five_second_cadence.sql.
--
-- Two changes, both about the gap between a price landing and the engine acting
-- on it. Measured against the venue: /v2/tickers recomputes every ~10s (all 122
-- XAUT contracts flip at once), so 0029 set both jobs to 5s — already twice the
-- rate of the source.
--
-- 1. apply every 2s, poll still every 5s.
--
--    apply never fetches; it reads the newest reply already stored. But the two
--    jobs are independent, so a reply could land just after apply ran and sit
--    there the best part of 5s. Running apply more often than the poll closes
--    that gap for DB reads alone — no extra venue calls, since the poll is what
--    talks to the venue and it is unchanged.
--
--    Safe: apply takes pg_try_advisory_xact_lock and returns at once if held, so
--    a pass that runs long is skipped rather than doubled, and each account is
--    still spaced by its own Refresh.
--
-- 2. The reply is only usable for 30s, not 180s.
--
--    180s was sized when the poll ran once a minute. With a 10s feed and a 5s
--    poll the newest reply is normally ~5s old, so 180s no longer means "recent"
--    — it means a run of failed polls can have the engine size a correction off
--    three-minute-old greeks. 30s leaves six poll cycles of slack and then stands
--    down, which is the right failure: no trade beats a trade on stale deltas.

select cron.schedule('delta-apply', '2 seconds', $$select public.apply_delta_strategy()$$);
