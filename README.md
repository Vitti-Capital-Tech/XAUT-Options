...
| Price rule | Rank against `entry_premium` | **The premium range is the rule.** No target of its own: the **cheapest** strike inside `[entry_premium_min, entry_premium_max]` ranks first, so a multi-pair entry ladders up from the floor ([`0072`](supabase/migrations/0072_the_premium_range_is_the_entry_rule.sql), [`0075`](supabase/migrations/0075_ladder_up_from_the_floor.sql)). `premium_buffer_pct` adds slack above the maximum; `max_pair_gap` refuses a pair whose legs are too far apart |
...
4. **Number of Pairs & the Premium Range**:
   - **`pairs_count`** (default **1**): The number of symmetric Call/Put pairs shorted at the session open. Both sides are ranked by the premium rule, then **matched by premium distance** — the closest remaining call/put pair on the board is taken first, then the next ([`0073`](supabase/migrations/0073_pair_the_legs_that_match.sql)). They used to be joined on *rank*, which is right while both sides offer the same ladder and wrong the moment they are different lengths: a range admitting three puts and one call took the top of each list, so a $2.83 call was sold against the richest $5.00 put with the $3.00 put one rank below it, unused. A pair's two legs are meant to sit the same distance out on either side of spot, and premium is the unit this strategy measures that in. Since [`0075`](supabase/migrations/0075_ladder_up_from_the_floor.sql) a pair is also refused outright when its two legs are further apart than `max_pair_gap`, and a call with no partner inside that is simply not sold — half a pair is a directional position this strategy never opens. An entry that opens fewer than this is **topped up** on later cycles rather than written off ([`0070`](supabase/migrations/0070_fill_the_pairs_the_window_asked_for.sql)); see [Filling the window's pairs](#filling-the-windows-pairs).
   - **`entry_premium_min`** & **`entry_premium_max`**: on a futures book these are the *whole* price rule. There is no `entry_premium` on this book any more ([`0072`](supabase/migrations/0072_the_premium_range_is_the_entry_rule.sql)) — the panel carried both, a target to rank against and a range to filter by, and once the range became hard the two could contradict each other outright. The configuration that was running was a **$6 target against a $3–$5 range**: aiming at a price no strike was allowed to be sold at.
     ...
     - **The target falls out of the range**: the **cheapest** in-range strike ranks first, the rest above it in order, so `pairs_count` above 1 gives a **ladder up from the floor** rather than pairs clustered around a separate number ([`0075`](supabase/migrations/0075_ladder_up_from_the_floor.sql)). Cheaper is further out of the money on a short strangle, so the ladder starts at the safe end of the band and moves in as it fills. It aimed at the ceiling between 0072 and 0075.
   - **`premium_buffer_pct`** (default **0**): slack above the maximum, as a percentage of it — the effective ceiling is `max × (1 + pct/100)`. A hard edge on a thin chain refuses entries over a few cents, and this lets the ceiling give a little rather than the entry fail. **Only the ceiling moves**: the floor is the risk edge, and it is what stops the book selling further and further out for nothing.

     The two rules are deliberately paired. Ranked from the floor, a strike inside the buffer is the *last* one reached, so the buffer is slack rather than a preference — ranked from the ceiling it would have been the first pick and every entry would have drifted up by the buffer.
   - **`max_pair_gap`** (default **$1**, 0 = off): the widest premium difference allowed between a pair's call and its put. Matching by premium distance ([`0073`](supabase/migrations/0073_pair_the_legs_that_match.sql)) fixed *which* put a call takes but could not refuse one — a lone $2.83 call against a lone $5.00 put is still the nearest pair on the board, and still short far more put than call. Since each pass takes the smallest gap left, one that fails the cap means every remaining combination does: the entry stops rather than skipping a pair, and the top-up tries again next cycle.
...
*the strike quoted closest to `entry_premium`* on a delta book, *the cheapest
