...
| State | `strategy_settings.last_acted`, one action per bar | `delta_strategy_settings` — roll/shift/re-entry budgets, touched strikes, session day, and on a futures book the window that owns the open book plus how many pairs it has opened |
...
| Price rule | n/a | `delta` ranks against `entry_premium`. `futures` has no target: the premium **range** is the whole entry rule, richest in-range strike first ([`0072`](../supabase/migrations/0072_the_premium_range_is_the_entry_rule.sql)) |
| Book shape | n/a | `futures` holds **one window's pairs on one expiry** — the window that opened the book owns it, anything on another contract is closed on sight, and the allocation is topped up toward `pairs_count` for as long as the window is open ([`0069`](../supabase/migrations/0069_one_window_one_book.sql)–[`0071`](../supabase/migrations/0071_count_the_pairs_that_are_actually_open.sql)) |
...
| 16 | Scope market-data calls to the underlying | Fetch the exchange, filter in the client | `/v2/tickers` unfiltered is 1079 symbols at 219 KB against 159 at 32 KB, once a minute. It also overflowed `page_size=1000` on `/v2/products` and silently dropped the 79 most recently listed contracts — the new strikes a chain most needs. The engines have asked this way since 0050; the browser was the straggler. |
| 17 | Every poller correlates by request id | Describe the reply by shape and age | `net._http_response` is shared by eight callers, and any description accurate today breaks when somebody adds the ninth. Describing already cost a book once — BTC's spot became XAUT's. `0065` closed the last describing caller. |
| 18 | Aggregate in the database, not the client | Fetch rows and count in JS | The admin panel's three per-account counts were pulling every row of `positions`, `fills` and `orders`. `account_counts()` counts off the indexes those tables already carry, runs `security invoker` so RLS scopes it unchanged, and is immune to a `db-max-rows` cap silently truncating the answer. |
